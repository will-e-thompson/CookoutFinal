...
namespace Cookout
{
    public partial class UserManager
    {
        ...
        public UserRegistration selectedUser;
        public UserRegistration currentUser;
        ...
        public async Task<ObservableCollection<UserRegistration>> GetUsersAsync(bool syncItems = false)
        {
            try
            {
                IEnumerable<UserRegistration> items = await userTable.ToEnumerableAsync();

                return new ObservableCollection<UserRegistration>(items);
            }
            catch (MobileServiceInvalidOperationException msioe)
            {
                Debug.WriteLine("Could not get users: {0}", new[] { msioe.Message });
            }
            catch (Exception e)
            {
                Debug.WriteLine("Could not get users:: {0}", new[] { e.Message });
            }
            return null;
        }        

        //public async Task SaveTaskAsync(UserRegistration user)
        //{
        //    try
        //    {
        //        if (user.Id == 0)
